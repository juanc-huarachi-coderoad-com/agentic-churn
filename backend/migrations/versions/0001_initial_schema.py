"""Initial schema — imports data-base/10-ddl-appendix.md verbatim.

Revision ID: 0001_initial_schema
Revises:
Create Date: 2026-08-13

data-base/10-ddl-appendix.md is the schema's source of truth (decisions/02-repo-and-
tooling.md §ORM and migrations); this revision is a straight import of it. The only
deviation from the document's literal text is the two role-password lines: the source
document uses psql's client-side ``:'var'`` variable substitution, which has no asyncpg
equivalent, so this migration substitutes the same values client-side in Python instead
(from app.config.Settings) before sending the DDL to the server — the resulting SQL sent
to Postgres is byte-identical to what psql would have sent. Every table, type, index,
grant, trigger, and function below is transcribed verbatim.
"""
...
DO $$ BEGIN
    ...
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'shredder_role') THEN
        CREATE ROLE shredder_role LOGIN PASSWORD '__SHREDDER_ROLE_PASSWORD__';
    END IF;
END $$;
...
def _split_sql_statements(sql: str) -> list[str]:
    """Split on top-level ``;`` only, treating anything between a ``$$`` pair (function
    bodies, the DO block) or inside a ``--`` line comment as opaque — several of the
    source document's own comments contain a literal ``;`` (e.g. "SHA-256 of the bearer
    token; the raw token is never stored"), which a naive split would misfire on.
    asyncpg's extended query protocol — used under SQLAlchemy/Alembic — rejects a single
    call containing more than one command, unlike psql's simple protocol, so the verbatim
    multi-statement DDL has to be split before executing it (see this file's module
    docstring)."""
    statements: list[str] = []
    current: list[str] = []
...
def upgrade() -> None:
    ddl = _DDL.replace(
        "__APP_ROLE_PASSWORD__", _escape_sql_literal(settings.app_role_password)
    ).replace("__SHREDDER_ROLE_PASSWORD__", _escape_sql_literal(settings.shredder_role_password))
    for statement in _split_sql_statements(ddl):
        op.execute(statement)


def downgrade() -> None:
    # Single big-bang first migration — reversed by dynamically dropping every table and
    # enum type this revision created (CASCADE handles the FKs/trigger), rather than
    # hand-writing DROP statements in exact reverse-dependency order for ~34 tables and
    # ~25 enum types. `DROP SCHEMA public CASCADE` would be simpler but also destroys
    # Alembic's own `alembic_version` bookkeeping table, which lives in the same schema
    # and must survive so Alembic can record the downgrade — this loop explicitly skips it.
    op.execute(
        """
        DO $$
        DECLARE
    ...
    op.execute("DROP FUNCTION IF EXISTS reject_update_delete() CASCADE;")
    op.execute("DROP FUNCTION IF EXISTS verify_hash_chain() CASCADE;")
    # A role can't be dropped while it still holds privileges (e.g. the SELECT grant on
    # alembic_version picked up incidentally by "GRANT SELECT ON ALL TABLES IN SCHEMA
    # public", since that table already existed when upgrade() ran) — DROP OWNED strips
    # them first.
    op.execute("DROP OWNED BY app_role CASCADE;")
    op.execute("DROP OWNED BY shredder_role CASCADE;")
    op.execute("DROP ROLE IF EXISTS app_role;")
    op.execute("DROP ROLE IF EXISTS shredder_role;")
