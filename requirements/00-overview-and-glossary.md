...
>
> The same mockup also shows two other things that are **not** part of this product and are deliberately absent: a **"Watch status: 24 accounts"** tile implying a multi-account portfolio view (out of scope per spec §13.1 boundary 1 — "one deployment serves one client company," `requirements/00-overview-and-glossary.md` §Scope), and a **"Sentiment: Negative / Frustrated"** label on the signal-stream card, implying an absolute sentiment scale (contradicts product principle **P7 — context over sentiment**: the Tone reader is baseline-relative to one stakeholder's own history, never a generic positive/negative/frustrated classification, `requirements/05-interpreters-readers.md` REQ-M5-06). Both are mockup artifacts inherited from a more generic dashboard template, not requirements — neither appears anywhere in `requirements/08-health-dashboard.md`'s component list, and neither should be built.
