...
| **Tech** | `AudioCollector` (implements the same `Collector` interface as M1 above) — Google Drive API discovery/download, OpenAI Whisper transcription, pyannote.ai hosted-API diarization (`pyannoteai-sdk`, research.md Decision 7's correction — moved off a locally-run `pyannote.audio`/PyTorch pipeline that alone dragged the deployed image to ~20GB), then one `LLMPort.generate_structured()` call (the same Anthropic adapter M5's readers already use) matching diarized speaker labels against the account's stakeholder roster, confidence-gated (`architecture/06-error-handling.md`'s resilience budget) |
