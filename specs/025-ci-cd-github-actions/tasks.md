...
- [ ] T001 `git mv workflows/ci.yml .github/workflows/ci.yml`; confirm the three existing jobs
...
- [ ] T002 Remove the now-empty `workflows/` directory if `ci.yml` was its only file (`rmdir
...
- [ ] T003 [US1] Push a branch containing only T001/T002's relocation to `origin` and open a PR
...
- [ ] T004 [US1] On that same PR, temporarily introduce one deliberate `ruff` violation (e.g. an
...
- [ ] T005 [US1] Merge the relocation PR to `main`; confirm the same three jobs run again on the
...
- [ ] T006 [US2] Run the one-time branch-protection configuration for `main` via `gh api`
...
- [ ] T007 [US2] Open a PR with a deliberately failing check (reuse T004's approach) and confirm
...
- [ ] T008 [US2] Fix the violation on that PR, push again, confirm the check turns green and the
...
- [ ] T009 [US3] Add a fourth job `publish` to `.github/workflows/ci.yml`, with
      `needs: [lint, type-check, test]` and
      `if: github.event_name == 'push' && github.ref == 'refs/heads/main'`
      (`research.md` Decision 1), and `permissions: { contents: read, packages: write }` at the
      job level.
- [ ] T010 [P] [US3] Within the `publish` job, add a `docker/login-action@v3` step authenticating
      against `ghcr.io` using `${{ secrets.GITHUB_TOKEN }}` (`research.md` Decision 2).
- [ ] T011 [P] [US3] Within the `publish` job, add a `docker/build-push-action@v6` step building
...
- [ ] T012 [P] [US3] Within the `publish` job, add a `docker/build-push-action@v6` step building
      `frontend/Dockerfile` (context `frontend/`) and pushing to
      `ghcr.io/${{ github.repository_owner }}/agentic-churn-web:${{ github.sha }}`
      (`research.md` Decision 3).
- [ ] T013 [US3] Push this change through a PR (subject to US1/US2's now-active gates), merge to
...
- [ ] T014 [US3] Confirm the negative case (FR-006): push a commit with a deliberately failing
...
- [ ] T016 [P] Add a new row for feature 025 to `specs/ROADMAP.md`'s Status table and a dated Log
      entry, following the exact format of the existing 001–024 rows/entries, once T001–T014 are
      verified complete.
- [ ] T017 Run `quickstart.md`'s full validation sequence end to end (all four scenarios) as a
